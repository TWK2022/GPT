...
# -------------------------------------------------------------------------------------------------------------------- #
# 设置
parser = argparse.ArgumentParser(description='|llama类大模型peft微调|')
parser.add_argument('--data_path', default=r'data.json', type=str, help='|sft(.json)数据路径|')
parser.add_argument('--weight', default='chinese-alpaca-2-1.3b', type=str, help='|tokenizer和模型文件夹位置|')
...
parser.add_argument('--lr_start', default=0.00001, type=float, help='|初始学习率|')
...
assert os.path.exists(args.weight), f'! weight不存在:{args.weight} !'
print(f'| 加载基座模型:{args.weight} |')
# -------------------------------------------------------------------------------------------------------------------- #
# 程序
